...
public void db_upgradeDatabase(int ver)
{
	...
	else if (ver == 9)
	{
		...
		SQL_FastQuery(g_hDb, "CREATE TABLE IF NOT EXISTS `ck_checkpointsnew` (`steamid` varchar(32) NOT NULL, `mapname` varchar(32) NOT NULL, `cp` int(11) NOT NULL DEFAULT '0.0', `time` decimal(12, 6) NOT NULL DEFAULT '0.000000', `velStartXY` int(11) NOT NULL DEFAULT '0.0', `velStartXYZ` int(11) NOT NULL DEFAULT '0.0', `velStartZ` int(11) NOT NULL DEFAULT '0.0', `zonegroup` int(12) NOT NULL DEFAULT '0.0', PRIMARY KEY (`steamid`,`mapname`,`cp`,`zonegroup`)) DEFAULT CHARSET=utf8mb4;");
...
void LoopFloatDecimalTables()
{
	...
	{
		if (g_sDecimalTables[i][0][3] == 'c' && strcmp(g_sDecimalTables[i][1], "time") == 0)
		{
			int cp_count;
			if(!g_bhasStages)
