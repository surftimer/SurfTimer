...
public void SaveRecording(int client, int zgroup, int style)
{
	...
	strcopy(iHeader.FH_Time, sizeof(FileHeader::FH_Time), g_szFinalTime[client]);
	iHeader.FH_tickCount = GetArraySize(g_hRecording[client]);
	strcopy(iHeader.FH_Playername, sizeof(FileHeader::FH_Playername), szName);
...
public void PlayRecord(int client, int type, int style)
{
	...
	{
		int stage = type * -1;
		Format(g_szWrcpReplayTime[stage], sizeof(g_szWrcpReplayTime), "%s", iFileHeader.FH_Time);
		Format(g_szWrcpReplayName[stage], sizeof(g_szWrcpReplayName), "%s", iFileHeader.FH_Playername);
...
public void WriteRecordToDisk(const char[] sPath, FileHeader iFileHeader)
{
	...
	int iTickCount = view_as<int>(iFileHeader.FH_tickCount);
	WriteFileCell(hFile, iTickCount, 4);

	FrameInfo iFrame; 
	for (int i = 0; i < iTickCount; i++)
	{
		GetArrayArray(iFileHeader.FH_frames, i, view_as<int>(iFrame), sizeof(FrameInfo));
...
public void LoadRecordFromFile(const char[] path, FileHeader headerInfo, bool headerOnly)
{
