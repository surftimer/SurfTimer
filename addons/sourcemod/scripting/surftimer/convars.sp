...
void CreateConVars()
{
	...
	g_hConnectMsg = AutoExecConfig_CreateConVar("ck_connect_msg", "1", "on/off - Enables a player connect message with country tag");
	g_hAllowRoundEndCvar = AutoExecConfig_CreateConVar("ck_round_end", "0", "on/off - Allows to end the current round");
	g_hDisconnectMsg = AutoExecConfig_CreateConVar("ck_disconnect_msg", "1", "on/off - Enables a player disconnect message in chat");
	g_hMapEnd = AutoExecConfig_CreateConVar("ck_map_end", "1", "on/off - Allows map changes after the timelimit has run out (mp_timelimit must be greater than 0)");
	g_hColoredNames = AutoExecConfig_CreateConVar("ck_colored_chatnames", "1", "on/off Colors players names based on their rank in chat.");
	g_hNoClipS = AutoExecConfig_CreateConVar("ck_noclip", "1", "on/off - Allows players to use noclip");
	g_hGoToServer = AutoExecConfig_CreateConVar("ck_goto", "1", "on/off - Allows players to use the !goto command");
	g_hCommandToEnd = AutoExecConfig_CreateConVar("ck_end", "1", "on/off - Allows players to use the !end command");
	g_hCvarGodMode = AutoExecConfig_CreateConVar("ck_godmode", "1", "on/off - Godmode");
	g_hPauseServerside = AutoExecConfig_CreateConVar("ck_pause", "1", "on/off - Allows players to use the !pause command");
	g_hcvarRestore = AutoExecConfig_CreateConVar("ck_restore", "1", "on/off - Restoring of time and last position after reconnect");
	g_hAttackSpamProtection = AutoExecConfig_CreateConVar("ck_attack_spam_protection", "1", "on/off - max 40 shots; +5 new/extra shots per minute; 1 he/flash counts like 9 shots");
	g_hRadioCommands = AutoExecConfig_CreateConVar("ck_use_radio", "0", "on/off - Allows players to use radio commands");
	...
	g_hDynamicTimelimit = AutoExecConfig_CreateConVar("ck_dynamic_timelimit", "0", "on/off - Sets a suitable timelimit by calculating the average run time (This method requires ck_map_end 1, greater than 5 map times and a default timelimit in your server config for maps with less than 5 times");
	...
	g_hSpawnToStartZone = AutoExecConfig_CreateConVar("ck_spawn_to_start_zone", "1.0", "1 = Automatically spawn to the start zone when the client joins the team.");
	g_hSoundEnabled = AutoExecConfig_CreateConVar("ck_startzone_sound_enabled", "1.0", "Enable the sound after leaving the start zone.");
	...
	g_hForceCT = AutoExecConfig_CreateConVar("ck_force_players_ct", "0", "Forces all players to join the CT team.");
	...
	g_hMultiServerMapcycle = AutoExecConfig_CreateConVar("ck_multi_server_mapcycle", "0", "0 = Use mapcycle.txt to load servers maps, 1 = use configs/surftimer/multi_server_mapcycle.txt to load maps");
	g_hDBMapcycle = AutoExecConfig_CreateConVar("ck_db_mapcycle", "1", "0 = use non-db map cycles, 1 use maps from ck_maptier");
	g_hDoubleRestartCommand = AutoExecConfig_CreateConVar("ck_double_restart_command", "1", "(1 / 0) Requires 2 successive !r commands to restart the player to prevent accidental usage.");
	g_hBackupReplays = AutoExecConfig_CreateConVar("ck_replay_backup", "1", "(1 / 0) Back up replay files, when they are being replaced");
	g_hReplaceReplayTime = 	AutoExecConfig_CreateConVar("ck_replay_replace_faster", "1", "(1 / 0) Replace record bots if a players time is faster than the bot, even if the time is not a server record.");
	g_hTeleToStartWhenSettingsLoaded = AutoExecConfig_CreateConVar("ck_teleportclientstostart", "1", "(1 / 0) Teleport players automatically back to the start zone, when their settings have been loaded.");
	g_drDeleteSecurity = AutoExecConfig_CreateConVar("ck_dr_delete_security", "1", "(1 / 0) Enable/Disable delete security for !dr command");
	g_iAdminCountryTags = AutoExecConfig_CreateConVar("ck_admin_country_tags", "0", "(1 / 0) Enable/Disable country tags for admins");
	...
	g_bHintsRandomOrder = AutoExecConfig_CreateConVar("ck_hints_random_order", "1", "(1 / 0) Enable/Disable hints shown in a random order");
	g_hOverrideClantag = AutoExecConfig_CreateConVar("ck_override_clantag", "1", "Override player's clantag");

	g_hPointSystem = AutoExecConfig_CreateConVar("ck_point_system", "1", "on/off - Player point system");
	HookConVarChange(g_hPointSystem, OnSettingChanged);
	g_hPlayerSkinChange = AutoExecConfig_CreateConVar("ck_custom_models", "1", "on/off - Allows SurfTimer to change the models of players and bots");
	...
	g_hAutoBhopConVar = AutoExecConfig_CreateConVar("ck_auto_bhop", "1", "on/off - AutoBhop on surf_ maps");
	HookConVarChange(g_hAutoBhopConVar, OnSettingChanged);
	g_hCleanWeapons = AutoExecConfig_CreateConVar("ck_clean_weapons", "1", "on/off - Removes all weapons on the ground");
	HookConVarChange(g_hCleanWeapons, OnSettingChanged);
	g_hCountry = AutoExecConfig_CreateConVar("ck_country_tag", "1", "on/off - Country clan tag");
	HookConVarChange(g_hCountry, OnSettingChanged);
	g_hAutoRespawn = AutoExecConfig_CreateConVar("ck_autorespawn", "1", "on/off - Auto respawn");
	HookConVarChange(g_hAutoRespawn, OnSettingChanged);
	g_hCvarNoBlock = AutoExecConfig_CreateConVar("ck_noblock", "1", "on/off - Player no blocking");
	HookConVarChange(g_hCvarNoBlock, OnSettingChanged);
	g_hReplayBot = AutoExecConfig_CreateConVar("ck_replay_bot", "1", "on/off - Bots mimic the local map record");
	HookConVarChange(g_hReplayBot, OnSettingChanged);
	g_hBonusBot = AutoExecConfig_CreateConVar("ck_bonus_bot", "1", "on/off - Bots mimic the local bonus record");
	HookConVarChange(g_hBonusBot, OnSettingChanged);
	g_hInfoBot = AutoExecConfig_CreateConVar("ck_info_bot", "0", "on/off - provides information about nextmap and timeleft in his player name");
	HookConVarChange(g_hInfoBot, OnSettingChanged);
	g_hWrcpBot = AutoExecConfig_CreateConVar("ck_wrcp_bot", "1", "on/off - Bots mimic the local stage records");
	...
	// Map Setting ConVars
	g_hGravityFix = AutoExecConfig_CreateConVar("ck_gravityfix_enable", "1", "Enables/Disables trigger_gravity fix");
