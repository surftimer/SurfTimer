...
public void OnConnect(Database db, const char[] error, any data)
{
	...
	if (strcmp(szIdent, "mysql", false) == 0)
	{
		...
		if (g_bLogQueries)
		{
			LogToFile(g_szQueryFile, "OnConnect - szQuery: SET sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));");
		}
			...
			if (results[i].HasResults && results[i].FetchRow())
			{
				if (results[i].FetchInt(0) == 0)
				{
					if (queryData[i] == 3)
					{
						if (g_bLogQueries)
						{
							LogToFile(g_szQueryFile, "SQLTxn_CreateDatabaseSuccess - szQuery: %s", sql_createPlayertimesIndex);
						}
					...
					else
					{
						if (g_bLogQueries)
						{
							LogToFile(g_szQueryFile, "SQLTxn_CreateDatabaseSuccess - szQuery: %s", sql_createBonusIndex);
						}
...
public void sql_selectLatestRecordsCallback(Handle owner, Handle hndl, const char[] error, int userid)
{
	if (hndl == null)
	{
...
public void db_selectMapCurrentWR(Handle owner, Handle hndl, const char[] error, DataPack pack)
{
	if (hndl == null)
	{
