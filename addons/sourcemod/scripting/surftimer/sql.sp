...
public void db_viewRecordCheckpointSpeedsInMap()
{
	for (int k = 0; k < MAXZONEGROUPS; k++)
	{
		g_bCheckpointRecordSpeedsFound[k] = false;
		for (int i = 0; i < CPLIMIT; i++)
			g_fCheckpointSpeedServerRecord[k][i] = 0.0;
	}

	char szQuery[1028];
	Format(szQuery, 1028, sql_selectRecordCheckpointsSpeeds, g_szRecordMapSteamID, g_szMapName, g_szMapName );
	...
	if (SQL_HasResultSet(hndl))
	{
		...
		while (SQL_FetchRow(hndl))
		{
			...
			for (int i = 0; i < 35; i++)
			{
				...
				if (!g_bCheckpointRecordSpeedsFound[zonegroup] && g_fCheckpointSpeedServerRecord[zonegroup][i] >= 0.0)
					g_bCheckpointRecordSpeedsFound[zonegroup] = true;
			}
		}
	}
...
public void db_viewCheckpointsSpeeds(int client, char szSteamID[32], char szMapName[128])
{
	char szQuery[1024];
	Format(szQuery, 1024, sql_selectCheckpointsSpeeds, szMapName, szSteamID);
	SQL_TQuery(g_hDb, SQL_viewCheckpointsSpeedsCallback, szQuery, client, DBPrio_Low);
}

public void SQL_viewCheckpointsSpeedsCallback(Handle owner, Handle hndl, const char[] error, any client)
{
	// fluffys come back
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (SQL_viewCheckpointsSpeedsCallback): %s", error);
		if (!g_bSettingsLoaded[client])
			LoadClientSetting(client, g_iSettingToLoad[client]);
		return;
	}
	...
	if (SQL_HasResultSet(hndl))
	{
		...
		{
			zoneGrp = SQL_FetchInt(hndl, 0);
			g_bCheckpointsSpeedsFound[zoneGrp][client] = true;

			for (int i = 0; i < 35; i++)
			{
				g_fCheckpointSpeedsRecord[zoneGrp][client][i] = SQL_FetchFloat(hndl, i + 1);
			}
		}
	}
...
public void db_viewCheckpointsinZoneGroupCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	...
	if (SQL_HasResultSet(hndl) && SQL_FetchRow(hndl))
	{
		...
		db_viewCheckpointsSpeedsinZoneGroup(client, g_szSteamID[client], g_szMapName, zonegrp);
...
public void db_viewCheckpointsSpeedsinZoneGroup(int client, char szSteamID[32], char szMapName[128], int zonegroup)
{
	char szQuery[1024];
	Handle pack = CreateDataPack();
	WritePackCell(pack, client);
	...
	Format(szQuery, 1024, sql_selectCheckpointsSpeedsinZoneGroup, szMapName, szSteamID, zonegroup);
	SQL_TQuery(g_hDb, SQL_viewCheckpointsSpeedsinZoneGroupCallback, szQuery, pack, DBPrio_Low);
}

public void SQL_viewCheckpointsSpeedsinZoneGroupCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (SQL_viewCheckpointsSpeedsinZoneGroupCallback): %s", error);
		return;
	}
	...
	{
		g_bCheckpointsSpeedsFound[zonegrp][client] = false;
	}
}
...
public void SQL_updateCheckpointsCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	db_updateCheckpointSpeeds(client, g_szSteamID[client], zonegrp);

}

public void db_updateCheckpointSpeeds(int client, char szSteamID[32], int zGroup)
{
	Handle pack = CreateDataPack();
	WritePackCell(pack, client);
	WritePackCell(pack, zGroup);
...
public void db_deleteCheckpointsSpeeds()
{
	char szQuery[258];
	Format(szQuery, 258, sql_deleteCheckpointsSpeeds, g_szMapName);
	SQL_TQuery(g_hDb, SQL_deleteCheckpointsSpeedsCallback, szQuery, 1, DBPrio_Low);
}

public void SQL_deleteCheckpointsSpeedsCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (SQL_deleteCheckpointsSpeedsCallback): %s", error);
		return;
	}
}
